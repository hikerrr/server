      ...
      <form class="form">
        ...
        <div class="form-field">
          <label for="heading" class='form-label'>Blog <span class='red'> *</span></label>
            <select v-model='destination.blog._id' class='form-input'>
        ...
        <router-link to='/admin/dashboard/destinations'>
          <a href="/admin/dashboard/destinations" @click='deleteDestById' class='form-btn'>Delete</a>
        </router-link>
        <div class="loader">
...
export default {
  ...
  created() {
    this.destination = _.cloneDeep(this.$store.getters.destById(this.id));
  },
  methods: {
    isTourPresent(id) {
      for(let i = 0; i < this.destination.tours.length; i++) {
        if(this.destination.tours[i]._id === id) {
          return true;
        }
      }
      return false;
    ...
    deleteDestById() {
      this.$store.dispatch('deleteDestination', this.destination.linkName);
    },
    saveData(evt) {
      evt.preventDefault();
